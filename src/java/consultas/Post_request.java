 ...
 */
@WebServlet(name = "Post_request", urlPatterns = {"/Post_request"})
public class Post_request extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Articulo> posts = (List<Articulo>)sesion.createQuery("SELECT p FROM Articulo p ORDER BY p.fecha").list();
       
        request.getSession().setAttribute("posts", posts);//Con esto se lo mando al JSP

        getServletContext().getRequestDispatcher("/Blog.jsp").forward(request, response); //Y con esto le redirijo al JSP
